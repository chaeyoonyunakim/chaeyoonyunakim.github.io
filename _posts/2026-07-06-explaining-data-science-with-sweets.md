...
layout: post
type: reflection
title: "Candy, Sweets, and Jelly: Explaining Data Science at School (and Getting My Labels Wrong)"
date: 2026-07-06
...
## Starting with the job, and a Boolean question

The first slide I presented was this one — what my job actually looks like, stripped of jargon.

![The "Work!" slide: a dog sitting in a suitcase next to paper bags of assorted sweets, with bullet points: using a computer and internet, having a super sorting power, making decisions with supporting data](/assets/img/school-talk-work-slide.png)
...
I put a **binary classifier** first, because a Boolean answer is the simplest possible model output: **can I share this packet of sweets with a dog?** Yes or no.

Chocolate is toxic to dogs, so anything chocolate gets a hard **No**. The answer isn't a matter of opinion — it comes from evidence about what theobromine does to dogs. That's the third bullet point doing its work: *making decisions with supporting data*. (And for the record, the safe answer for the fruit gummies is also no — but the point was that the model draws a line, and the line comes from data, not vibes.)

For Year 1 and 2 at the academy, that was the whole lesson: a machine can learn to answer yes-or-no questions, and the answers have to come from evidence, because a wrong answer here really hurts someone (or some dog).

## Extending to a multi-class classifier: candy or chocolate?

From there I aimed to extend the example with a second classifier. Instead of two answers, sort everything on this slide — things they'd all recognise from the corner shop — into classes: **candy** or **chocolate**.
...
## Then a new label: what if the model wasn't trained for Haribos?

The last step of the arc was my favourite question: **what happens when the model meets something it wasn't trained for?** Haribos are neither chocolate nor, really, candy-the-hard-sugary-kind. A two-class model has nowhere to put them — it will confidently shove a Goldbear into whichever class is nearest, and be wrong.

So I introduced a third label, **"jelly"**, for the Haribo-type gummies — to show that sometimes the right fix isn't a better model, it's admitting your label set doesn't cover the world.

Good idea in principle. In practice, it added a second layer of confusion on top of one I'd already created without noticing.
...
The irony is that this is *exactly* the kind of failure I deal with professionally. I stood in front of three year groups explaining that classifiers are only as good as their labels — while using labels that didn't match the domain vocabulary of my users. Even the part of the talk *about* incomplete label sets introduced a label with the wrong meaning.
...
**4. The corrected taxonomy was sitting on my own slide.** *Sweets* / *chocolate* / and, for the not-trained-for class, *gummies* or just *Haribo* — a brand name that every child in Britain resolves to precisely the right concept. The best label is the one your users already use.
